...
<HTML>
<HEAD>
<TITLE>Сводная таблица дисциплин, которые ведет кафедра</TITLE>
...
<?php
    $time_b = getmicrotime();
    $Departs = FetchArrays("select Reduction, CodeOfDepart from department where active=1 order by CodeOfFaculty, Reduction");
    $time_e = getmicrotime();	
...
<td align="center" width="80%"><em class="h1">Сводная таблица дисциплин, которые ведет кафедра <?=$depname?></em></td>
<td align="center" width="10%">&nbsp;</td>
</tr>
<?php
...
<?php
	...
	if ($_GET['sem']==1	)
			...
			left join schplanitems as si on sch.CodeOfSchPlan=si.CodeOfSchPlan
				where sch.CodeOfDepart='$_GET[dep]' and (pl.FixDate is NULL or pl.FixDate='0000-00-00') 
...
<?php
	...
	if ($_GET['sem']==1)
			...
			WHERE sch.CodeOfDepart='$_GET[dep]' and (pl.FixDate is NULL or pl.FixDate='0000-00-00') and NumbOfSemestr in (1,3,5,7,9,11)
		...
		$q = "SELECT pl.CodeOfPlan, DisName, NumbOfSemestr, LectSem, PractSem, LabSem, test, exam, sp.MinistryCode 
			...
			left join schplanitems as si on sch.CodeOfSchPlan=si.CodeOfSchPlan 
			left join specials as sp on pl.CodeOfSpecial=sp.CodeOfSpecial 
			WHERE sch.CodeOfDepart='$_GET[dep]' and (pl.FixDate is NULL or pl.FixDate='0000-00-00') and NumbOfSemestr in (2,4,6,8,10,12)
			order by sp.MinistryCode, NumbOfSemestr, DisName";

    	$Specials = FetchArrays($q); 
    	foreach ($Specials as $k=>$v) {
		$q = "Select GroupCount from streams as st left join plans as pl on st.CodeOfPlan=pl.CodeOfPlan 
			where st.CodeOfPlan='$v[CodeOfPlan]' and Kurs=round($v[NumbOfSemestr]/2)";
