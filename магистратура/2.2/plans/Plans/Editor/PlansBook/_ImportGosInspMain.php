			...
			if ($key1=="СтрокиПлана"){
				...
				foreach($value1 as $key2=>$value2){
					...
					foreach($value2->attributes() as $attributeskey2 => $attributesvalue2){
						...
						if ($attributeskey2=='ДисцплинаДляРазделов')
							$flag=1;
					}
					if ($flag==1 && $planDis[$i]['depart']) $dep_default = $planDis[$i]['depart'];
					if ($flag==0 && !$planDis[$i]['depart']) $planDis[$i]['depart'] = $dep_default;
					if ($planDis[$i]['discip'] && $planDis[$i]['depart'] && $planDis[$i]['cicle'] && $flag==0){
