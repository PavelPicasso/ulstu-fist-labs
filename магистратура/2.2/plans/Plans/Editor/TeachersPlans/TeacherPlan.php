...
$sql = "select distinct(plans.CodeOfPlan) as cp ".
        "FROM (schedplan, teachersDiscips, schplanitems) ".
        "LEFT JOIN plans ON (plans.CodeOfPlan = schedplan.CodeOfPlan) ".
        "WHERE (plans.Date IS NULL OR plans.Date = '0000-00-00') ".
        "AND teachersDiscips.CodeOfTeacher = {$CodeOfTeacher} ".
...
DisplayPageTitle("../down1.html","Объемы учебной нагрузки преподавателя","Файл отчета");
...
include ("ExpTchPlFunc.php");
$start = "TchPlRTFParts/StartPage.rtf";
$colonEnd = "TchPlRTFParts/ColonTitulEnd.rtf";
$startTbl = "TchPlRTFParts/StartTbl2.rtf";
$div = "TchPlRTFParts/DivTable2.rtf";
$AddQ = "";
...
if (strcmp($AddQ,"")!=0){ $AddQ .= ") ";}
$q = "select department.DepName, department.Reduction, faculty.DeanSignature, faculty.Reduction, department.ZavSignature, teachers.TeacherName ".
    "from department, faculty, teachers  ".
    "where teachers.CodeOfTeacher = {$CodeOfTeacher} ".
...
echo "<P ALIGN='CENTER'><h2>{$TeacherName}: ";
//Подбор имени файла---------------------------
$filename="../../../Exported/TchPl";
$i=0;
while ( file_exists ($filename."_".$i.".rtf")){ $i++;}
$PrintName = "TchPl_".$i.".rtf";
$filename .= "_".$i.".rtf";
//---------------------------------------------
$fl = fopen ($filename,"w")
or die("Невозможно создать файл  $filename.<BR>");
FileCopy($fl,$start);
fwrite ($fl," Объемы учебной нагрузки преподавателя $TeacherName ");
FileCopy($fl,$colonEnd);
fwrite ($fl,"{\\b\\f51\\fs28  преподавателя }");
fwrite ($fl,"{\\b\\f51\\fs28\\ul $TeacherName}");
...
$fromC = " streams, plans ";
$selectC = "distinct plans.CodeOfPlan, ".
...
for ($semestr = 1; $semestr<=2; $semestr++) {
    ...
    fwrite ($fl,"{\\b0\\f51\\fs28 Утверждаю \\par}{\\b0\\f51\\fs28 Зав. кафедрой _______________ \\par}{\\b0\\f51\\fs28 «___»________________20___г. \\par}{\\par}");
    if ($semestr == 1){
        fwrite ($fl,"{\\b\\f51\\fs28 Осенний семестр}");
    }else{
        fwrite ($fl,"{\\b\\f51\\fs28 Весенний семестр}");
    }
    ...
    foreach ($Streams as $ks =>$vs) {
        ...
        $dq = "SELECT distinct ".
            "teachersDiscips.CodeOfTeacherDiscip, ".
            "cicles.CodeOfCicle, ".
            "cicles.CicReduction, ".
            "disciplins.DisName, ".
            "disciplins.CodeOfDiscipline, ".
            "schedplan.CodeOfSchPlan, ".
            "schplanitems.CodeOfSchPlanItem, ".
            "undercicles.CodeOfUnderCicle, ".
            "schedplan.UndCicCode ".
            "FROM teachersDiscips, cicles, disciplins, schedplan, undercicles, schplanitems ".
            "WHERE teachersDiscips.CodeOfTeacher = {$CodeOfTeacher} ".
            "AND schplanitems.CodeOfSchPlanItem = teachersDiscips.CodeOfSchPlanItem ".
            "AND schedplan.CodeOfSchPlan = schplanitems.CodeOfSchPlan ".
            "AND disciplins.CodeOfDiscipline = schedplan.CodeOfDiscipline ".
            "AND cicles.CodeOfCicle = schedplan.CodeOfCicle ".
            "AND undercicles.CodeOfUnderCicle = schedplan.CodeOfUndCicle ".
        ...
        foreach ($Discips as $kd => $vd) {
            ...
            $schitem = FetchFirstRow("select distinct * \n" .
            " from schplanitems, teachersDiscips \n" .
            " where \n" .
            " schplanitems.CodeOfSchPlan='$vd[CodeOfSchPlan]' and \n" .
            " schplanitems.CodeOfSchPlanItem=teachersDiscips.CodeOfSchPlanItem and \n" .
            " teachersDiscips.CodeOfTeacher='{$CodeOfTeacher}' and \n" .
            " schplanitems.NumbOfSemestr='$Term' \n");
            if (!empty($schitem)){
                $newTeachersDis = $schitem["CodeOfTeacherDiscip"];
                if (in_array($newTeachersDis, $teachersDis)) {
                    continue;
                } else {
                    if (!empty($newTeachersDis)) {
                        $teachersDis[] = $newTeachersDis;
                    }
                }
                ...
                $DisCode = $vd["CodeOfDiscipline"]; //Код дисциплины
                $FPrnCode = $FCode;
