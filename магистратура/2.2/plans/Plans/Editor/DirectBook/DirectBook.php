...
<SCRIPT language="JavaScript">
        <!--
            ...
            { if(confirm(" Отмеченные строки будут безвозвратно удалены из базы данных. Удалить их?"))
                {return ('DoDelDirect.php');
                }
              return('DirectBook.php');
            }
        // -->
        </script></HEAD>
...
<?php
   ...
   if ($_SESSION["statusCode"] == 0){ $q = "select * from directions order by CodeOfFaculty, CodeOfDepart, CodeOfDegree, DirName";}
   else{ $q = "select * from directions where CodeOfFaculty=".$_SESSION["statusCode"]." order by CodeOfFaculty, CodeOfDepart, CodeOfDegree, DirName";}
   ...
   while ($row = mysql_fetch_object($result)){
      ...
      echo "<TR>\n";
      echo "<TD align='center'><INPUT TYPE='CHECKBOX' NAME='flag[$DirCode]' VALUE=\"$DirCode\"></TD>\n";
      echo "<TD align='center'><INPUT TYPE=TEXT NAME=\"MinCode$DirCode\"   SIZE='6' MAXLENGTH=6 VALUE='".trim($MinCode)."'\n";
      echo "              onChange=\"Validator(this)\"></INPUT></TD>\n";
      echo "<TD align='center'><INPUT TYPE=TEXT NAME=\"Dir$DirCode\"   SIZE='50' MAXLENGTH=150 VALUE='".trim($Dir)."'\n";
      echo "              onChange=\"FillArrChenge(this)\"></INPUT></TD>\n";
      echo "<TD align='center'><SELECT NAME=\"Fac$DirCode\" onChange=\"FillArrChenge(this)\">\n";
      reset($FacArray);
      reset($CodeFacArray);
      ...
      echo "</SELECT></TD>\n";
      //Выбор кафедры
      echo "<TD><SELECT NAME=\"Depart$DirCode\" onChange=\"FillArrChenge(this)\">\n";
      ...
      echo "</SELECT></TD>\n";
      echo "<TD align='center'><SELECT NAME=\"Type$DirCode\" onChange=\"FillArrChenge(this)\">\n";
      reset($TypeArray);
      reset($CodeTypeArray);
...
<TD><CENTER><INPUT TYPE='SUBMIT' NAME='Edit' VALUE='Внести изменения'
            onClick="fed.action='DoEditDirectInB.php'"></INPUT></CENTER></TD>
...
<TD><CENTER><input type=submit           VALUE='Добавить в справочник новую запись' 
            onClick="fed.action='NewDirect.php'"> 
</CENTER></TD>
