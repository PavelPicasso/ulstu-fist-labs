...
<em class='h1'><center>Утвержденные ланы</center></em>
...
<TH><strong>Дата утверждения</strong></TH>
</TR>
<?php  
   $send="/Plans/planFull.php?plan=";
   include("../../cfg.php");
   ...
   $Dt = date("Y/m/d");
   $sq = "select plans.CodeOfPlan, plans.PlnName, plans.Date, plans.CodeOfDirect, plans.CodeOfSpecial, plans.CodeOfSpecialization from  plans where plans.Date is not NULL and (plans.DateArchive is NULL or plans.DateArchive='0000-00-00') order by plans.Date";
   ...
   while ($row = mysql_fetch_object($result)) {
      ...
      $resDt = mysql_query("select specialslimit.TotalHours from specialslimit, plans
               ...
               specialslimit.CodeOfSpecial = plans.CodeOfSpecial", $Connection)
         or die("Unable to execute query:".mysql_errno().": ".mysql_error()."<BR>");
      ...
      $mon = substr($row->Date,5,2); 
      $year = substr($row->Date,0,4);
      echo "<TH ALIGN='CENTER'>$day.$mon.$year</TH>";
      echo "</TR>";
      mysql_free_result($resSum);
   }
   /*  */
