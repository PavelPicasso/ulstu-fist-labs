...
<em class='h1'><center>Планы измененные за сутки</center></em>
...
<TH><strong>Добавлено за день</strong></TH>
...
<?php  
   ...
   $Dt = date("Y/m/d");
   $result = mysql_query("select plans.CodeOfPlan, plans.CodeOfDirect, plans.CodeOfSpecial, plans.CodeOfSpecialization, plans.PlnName, control.CPositions from  plans, control  where control.CodeOfPlan=plans.CodeOfPlan and control.CPositions!=0 and control.CDate='".$Dt."'and plans.Date is NULL", $Connection)
      or die("Unable to execute query:".mysql_errno().": ".mysql_error()."<BR>");
   while ($row = mysql_fetch_object($result)) {
      ...
      echo "<TH ALIGN='CENTER'>$DayAdd</TH>";
      }
