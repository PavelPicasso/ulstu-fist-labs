...
<form method="post" action="DoNewSpz.php" enctype="application/x-www-form-urlencoded" metod="post">
...
<?php
   ...
   echo "<TD align='center'><SELECT NAME=\"DS\">\n";
   ...
   while ($row = mysql_fetch_object($result)){
      echo "<OPTION VALUE='".$row->CodeOfDirect.",0'>".$row->MinistryCode." / ------------\n";
      ...
      while ($rowSpc = mysql_fetch_object($resultSpc)){
         echo "<OPTION VALUE='".$row->CodeOfDirect.",".$rowSpc->CodeOfSpecial."'>".$row->MinistryCode." / ".$rowSpc->MinistryCode."\n";
      }
   }
   $resultSpc = mysql_query("select MinistryCode, CodeOfSpecial from specials where CoDeOfDirect is NULL order by CodeOfFaculty, CodeOfDepart", $Connection)
      or die("Unable to execute query:".mysql_errno().": ".mysql_error()."<BR>");
   while ($rowSpc = mysql_fetch_object($resultSpc)){
      echo "<OPTION VALUE='0,".$rowSpc->CodeOfSpecial."'>------------ / ".$rowSpc->MinistryCode."\n";
   }
...
<TD align='center'><INPUT TYPE=TEXT NAME='SpzN' MAXLENGTH=150 SIZE=50></INPUT></TD></TR></TABLE></td></tr></table><BR><br><table  class='ramka' border="0" cellpadding="0" cellspacing="0" align="center">
<tr><td cellpadding="0" cellspacing="0">
<TABLE  class='table' BORDER=0 ALIGN=CENTER CELLSPACING=1 CELLPADDING=1 WIDTH='100%'>
<TR>
...
<?php
   ...
   echo ">Вернутся к справочнику специализаций &nbsp;&nbsp;</TD>\n";
   echo "<TD><INPUT TYPE='RADIO' NAME='shift' VALUE='1'";
   if($sh==1){echo "CHECKED ";}
   echo ">Ввести несколько новых специализаций</TD>\n";
?>
</TR>
</TABLE>
<BR>
