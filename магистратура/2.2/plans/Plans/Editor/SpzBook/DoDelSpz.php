<?php
   ...
   while ( $delflag = each($flag)){
      $result = mysql_query("SELECT * from plans where CodeOfSpecialization=".$delflag[1], $Connection)
         or die("Unable to execute query:".mysql_errno().": ".mysql_error());
      if (mysql_fetch_row($result)){$Ok = 0;}
      else{
         mysql_query("delete from specializations where CodeOfSpecialization=".$delflag[1], $Connection)
            or die("Unable to execute query:".mysql_errno().": ".mysql_error());
      }
      mysql_free_result($result);
   }
   mysql_close($Connection);
   if ($Ok){ Header ("Location: SpzBook.php");}
