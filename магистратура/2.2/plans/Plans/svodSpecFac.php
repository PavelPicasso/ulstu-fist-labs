...
<HTML>
<HEAD>
<TITLE>Сводная таблица дисциплин специальностей факультета</TITLE>
...
<br>
<form action="svodSpecFac.php" method="get">
<table border=0 width="90%" align="center">
<tr>
<td colspan=2><strong>Выберите кафедру</strong>&nbsp;&nbsp;<select name="dep">
<?php
    $time_b = getmicrotime();
    $Departs = FetchArrays("select d.Reduction, CodeOfDepart, d.CodeOfFaculty, f.Reduction as fn 
		from department as d left join faculty as f on d.CodeOfFaculty=f.CodeOfFaculty
		where active=1 order by d.CodeOfFaculty, d.Reduction");
    $time_e = getmicrotime();	
    foreach ($Departs as $k => $v){
	if ($_GET['dep']==$v[CodeOfDepart]){
		echo "<OPTION value='$v[CodeOfDepart]' selected>$v[Reduction]</OPTION>\n";
		$depname = $v[Reduction];
		$fac_id=$v[CodeOfFaculty];
		$facname=$v[fn];
	}
	else
...
<td align="center" width="80%"><em class="h1">Сводная таблица дисциплин специальностей факультета <?=$facname?>, которые ведет кафедра <?=$depname?></em></td>
<td align="center" width="10%">&nbsp;</td>
</tr>
<?php
...
<?php
	...
	if ($_GET['sem']==1	)
			...
			left join schplanitems as si on sch.CodeOfSchPlan=si.CodeOfSchPlan
				where sch.CodeOfDepart='$_GET[dep]' and s.CodeOfFaculty=$fac_id and (pl.FixDate is NULL or pl.FixDate='0000-00-00') 
...
<?php
    	...
    	$time_b = getmicrotime();
	if ($_GET['sem']==1)
    		$q = "SELECT pl.CodeOfPlan, DisName, NumbOfSemestr, LectSem, PractSem, LabSem, test, exam, sp.MinistryCode 
			...
			WHERE sch.CodeOfDepart='$_GET[dep]' and (pl.FixDate is NULL or pl.FixDate='0000-00-00') and 
			NumbOfSemestr in (1,3,5,7,9,11) and sp.CodeOfFaculty=$fac_id
			order by sp.MinistryCode, NumbOfSemestr, DisName";
	else
		$q = "SELECT pl.CodeOfPlan, DisName, NumbOfSemestr, LectSem, PractSem, LabSem, test, exam, sp.MinistryCode 
			FROM `schedplan` as sch 
			left join plans as pl on sch.CodeOfPlan=pl.CodeOfPlan 
			...
			left join specials as sp on pl.CodeOfSpecial=sp.CodeOfSpecial 
			WHERE sch.CodeOfDepart='$_GET[dep]' and (pl.FixDate is NULL or pl.FixDate='0000-00-00') and 
			NumbOfSemestr in (2,4,6,8,10,12) and sp.CodeOffaculty=$fac_id
			order by sp.MinistryCode, NumbOfSemestr, DisName";
		...
		if ($Groups){
        	...
        	echo "<td  align='center'>$depname</td>\n";
