...
void Zvezda(HDC hdc, int x, int y, int rad, int depth)
{
	...
	if (depth > 0) 
	{
		Zvezda(hdc, x, y - rad , rad / 2, depth - 1);
		Zvezda(hdc, x + rad, y, rad / 2, depth - 1);
		Zvezda(hdc, x, y + rad, rad / 2, depth - 1);
		Zvezda(hdc, x - rad, y, rad / 2, depth - 1);
	}
}
    ...
    case WM_PAINT:
        {
			...
			HPEN hPen;
			hPen = CreatePen(PS_SOLID, 3, RGB(35, 177, 77));
			SelectObject(hdc,hPen);
			Zvezda(hdc, 500, 300, 160, 3);
