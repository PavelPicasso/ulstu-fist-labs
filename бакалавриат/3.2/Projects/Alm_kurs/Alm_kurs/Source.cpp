...
void graf() {
	...
	while (1) {
		...
		if (state[0] == 0 && state[1] == 0 && state[2] == 0 && state[3] == 0 && !x[0] && !x[1] && x[3] && !x[15]) {
			cout << "S0 -> S2\noutput: -\n";
			Sgraf << "S0 -> S2\noutput: -\n";
			state[0] = 0; state[1] = 0; state[2] = 1; state[3] = 0;
		}
